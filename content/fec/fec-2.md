...
我们不妨用更统一更正式的形式表示分组码：对于分组码，**原始消息**和**编码之后的消息**都由 $q$ 个符号的 **“符号表”** 组成。原始消息长度 $r$，经过编码之后的消息（码字）长度 $n$。
...
编码算法可以看成消息集和码字集之间的映射。或者说将 $r$ 维向量空间中映射到 $n$ 维向量空间（这个说法不是很严谨）。
...
严格来说，那些有对应消息的向量被称为码字（Codeword），而那些没有被映射的就称为字（Word），所谓的编码算法（Code）就是这个映射。那么现在设计一个编码算法需要解决的问题就是：

- 如何在所有 Word 的集合中选择码字？
- 如何设计消息和码字之间的映射？
- 如果发生错误，如何获得正确的原始消息？

比如，我们用某种办法随机选出来 $q^k$ 个码字然后和消息随机做个一一映射，这其实就是一种编码算法。但是这么做解码器就难办了：如果接收到的码字有错，想要还原原始的消息就有些抓瞎。另外随机选这么一组映射，也没有办法回答“这种编码方式的纠错是多大？能否安全纠多少比特错误？”诸如此类的问题。

其实所有的编码方式都在为解码器能够简单高效计算而工作。为此，我们需要对消息和符号集合再增加一些数学。
...
现在再回到分组码，我们试图把分组码和伽罗华域结合在一起：前面我们说到分组码的“字符集”是：$\Sigma_q$，当 $q = 2$ 时这个字符集就是二进制数据 0/1，那么其实这就是 $GF(2)$ 域。此外分组码中我们的待编码和编码之后的数据是两个向量 $(d_{k-1}, d_{k-2}, ..., d_1, d_0)$ 和 $(c_{n-1}, c_{n-2}, ..., c_1, c_0)$。现在我们把这两个向量看成两个多项式：
...
在汉明码的时候，我们提到了它的编码过程可以表示为矩阵的形式：$c = d \cdot \bm{G}$。类比于此，我们使用一个叫生成多项式的 $g(x)$ 的东西替代生成矩阵，我们定义的编码过程就变成了：

$$
c(x) = d(x) \cdot g(x)
$$

那么恭喜我们，我们发明了多项式编码（Polynomial Codes）。实际上根据其工作方式，分组码可以分为：**线性码**、**多项式码**、**循环码**、**BCH 码**、**汉明码**、**里德·所罗门（Reed-Solomon/RS）码**等。上述各种编码的层级如图所示：

![分组码分类与继承关系](./block-codes-hierarchy.png)

我们先来介绍 BCH 码其上的各种编码。至于 BCH 码我们会在下一篇文章详细介绍。

### 线性码

线性码（线性分组码）意味着它具有代数（或者叫线性）的结构，合法码字之间的加法和乘法运算之后仍然是合法码字。线性码比其他码更高效，且更易于编码和解码。

线性码的编码可用生成矩阵 $\bm{G}$ 来描述，解码可用校验矩阵 $\bm{H}$ 来描述。我们在上一篇介绍的汉明码就是一种线性码。
...
当 $G$ 具有分块矩阵形式 $G = [I_k | P]$，其中 $I_k$ 是 $k \times k$ 单位矩阵，$P$ 是某个 $k \times (n-k)$ 矩阵时，我们称 $G$ 处于标准形式或系统形式。如果生成矩阵是系统形式，我们称该码为**系统码**

如果 $G$ 是系统码形式，那么原始消息出现在码字中，“校验符号”附加在消息后面。系统形式有一些优点：消息出现在码字中，因此不需要矩阵求逆即可从码字得到原始消息。所以本文后续讨论的所有码都是**系统码**。

![系统码示意](./system-code.png)

#### 校验矩阵

校验矩阵（也称奇偶校验矩阵）是接收端或解码器用于检查码字的矩阵。如果接收到的是有效码字，则有：
...
也就是说，**伴随式只取决于错误**，这一点在解码中非常重要。如果存在错误，我们可以仅通过**伴随式**来指示错误。
...
多项式码是一类线性码，其有效码字集合由可被给定多项式整除的多项式组成。这个多项式 $g(x)$ 称为**生成多项式**。$g(x)$ 是 $GF(q)$ 上次数为 $m$ 的多项式：

$$
g(x) = g_{m}x^m + g_{m-1}x^{m-1} + ... + g_1x + g_0
$$
...
可通过将接收到的消息除以生成多项式来检测错误。如果结果为零，则消息是一个有效码字；如果有错误，将得到一个非零余数。伴随式多项式 $S(x)$ 是将多项式 $v(x)$ 除以生成多项式 $g(x)$ 的余数：

$$
\begin{align}S(x) &= v(x) \mod{g(x)} \\
...
Singleton 界以美国数学家 _Richard Collom Singleton_ 命名，是对任意块码 $C$（块长 $n$、最小距离 $d$）大小的相对粗略的上界。对于 $GF(q)$ 上的线性码，Singleton 界意味着：

$$
d\leq n-k+1
$$

Reed–Solomon 码就是达到 Singleton 界等号的例子。

### 汉明距离

**汉明距离**是指两个等长字符串或向量在对应位置上不同符号的数量。
...
其中 $d(x, y)$ 是 $x$ 与 $y$ 的汉明距离。

对于**线性码**，要找到码字之间的最小距离，只需查看非零码字。具有最小重量的非零码字与零码字的距离就是码的最小距离。

### Hamming 界

Hamming 界是对任意块码参数的限制。
